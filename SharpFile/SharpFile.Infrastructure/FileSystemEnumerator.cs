...
=============================================================================*/
	...
	/// </summary>
	[Obsolete("This method purports to be faster, but I haven't seen any massive increases in speed compared to GetFiles().")]
	public sealed class FileSystemEnumerator : IDisposable {
		...
		public IEnumerable<SharpFile.IO.FileSystemInfo> Matches() {
				...
				if (!handle.IsInvalid) {
					do {
						...
						if ((findData.fileAttributes & (int)FileAttributes.Directory) != 0) {
							yield return new SharpFile.IO.DirectoryInfo(new DirectoryInfo(Path.Combine(path, findData.fileName)));
							...
							foreach (Regex fileSpec in m_fileSpecs) {
								...
								if (fileSpec.IsMatch(findData.fileName))
									yield return new SharpFile.IO.FileInfo(new FileInfo(Path.Combine(path, findData.fileName)));
							}
						}
