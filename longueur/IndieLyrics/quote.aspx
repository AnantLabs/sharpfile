			...
			<div id="comments">
				...
				<span class="text">
					...
					<div id='editCommentTextArea_<%# DataBinder.Eval(Container.DataItem, "CommentID") %>' style="DISPLAY: none">
						<textarea id="editedCommentText" cols="48" rows="3" runat="server" style='<%# SiteUser.IsUserAuthorized((SiteUser)Session[Constants.CurrentUser], Convert.ToInt32(DataBinder.Eval(Container.DataItem, "UserID"))) ? "" : "DISPLAY: none;" %>'><%# DataBinder.Eval(Container.DataItem, "CommentText") %></textarea>
						<div class="actions"><asp:ImageButton ID="saveBtn" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "CommentID") %>' CommandName="save" ImageUrl="~/IndieLyrics/Images/comment_save.png" AlternateText="Save Comment" ToolTip="Save Comment" runat="server" Visible='<%# SiteUser.IsUserAuthorized((SiteUser)Session[Constants.CurrentUser], Convert.ToInt32(DataBinder.Eval(Container.DataItem, "UserID"))) %>' /></div>
						<p>&nbsp;</p>
					</div>
